...
UserVoiceSSO.prototype.setDefault = function(key, value) {
  if(_.isObject(key)) {
    // Given an object
    this.defaults = _.extend(this.default, key);
  } else {
    // Given key and value
...
UserVoiceSSO.prototype.createToken = function(user) {
  ...
  var iv, password, saltedHash, token, cipher;
  ...
  user = new Buffer(JSON.stringify(user), 'utf-8');

  // IV that uservoice uses is apparently 'OpenSSL for Ruby'
  iv = new Buffer('OpenSSL for Ruby', 'utf-8');
  ...
  saltedHash =
    crypto.createHash('sha1')
     .update((this.ssoKey + this.subdomin), 'utf-8')
     .digest()
     .slice(0, 16);
...
var sso = new UserVoiceSSO('verbling-teacher', '53f03c0787e78dbbc71ed290758f9f55');

console.log(sso.createToken({guid:'1234'}))